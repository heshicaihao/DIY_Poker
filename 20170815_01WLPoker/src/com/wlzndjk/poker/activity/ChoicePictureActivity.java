...
import com.wlzndjk.poker.adapter.ChoicePictureGridViewHolder;
import com.wlzndjk.poker.adapter.ChoicePictureListViewHolder;
import com.wlzndjk.poker.adapter.SelectPicViewHolder;
import com.wlzndjk.poker.base.BaseActivity;
import com.wlzndjk.poker.bean.ImageBean;
import com.wlzndjk.poker.bean.SelectpicBean;
import com.wlzndjk.poker.bean.UserBean;
import com.wlzndjk.poker.cache.ACache;
//import com.wlzndjk.poker.cache.AutomaticUtil;
...
import com.wlzndjk.poker.dialog.TemplateCustomDialog;
import com.wlzndjk.poker.net.NetHelper;
import com.wlzndjk.poker.picdeal.DealTask;
import com.wlzndjk.poker.utils.AndroidUtils;
import com.wlzndjk.poker.utils.BitmapHelp;
import com.wlzndjk.poker.utils.DateFormatUtils;
import com.wlzndjk.poker.utils.FileUtil;
import com.wlzndjk.poker.utils.GUID;
import com.wlzndjk.poker.utils.JSONUtil;
import com.wlzndjk.poker.utils.LogUtils;
import com.wlzndjk.poker.utils.ServiceUtils;
import com.wlzndjk.poker.utils.StringUtils;
import com.wlzndjk.poker.utils.ToastUtils;
import com.wlzndjk.poker.widget.ErrorHintView;
import com.wlzndjk.poker.widget.ErrorHintView.OperateListener;
import com.wlzndjk.poker.widget.MyGridview;
...
public class ChoicePictureActivity extends BaseActivity implements
	...
	class ChildGridAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Glide.with(mContext).load(mPath).placeholder(R.drawable.blank_bg)
	...
	class SelectPicAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Glide.with(mContext).load(path).placeholder(R.drawable.blank_bg)
					.into(holder.mImageView);
			return convertView;
		}
	}
	...
	private void resolveTemplateInfo(byte[] responseBody) {
		try {
			JSONObject result = JSONUtil.resolveResult(responseBody);
			if (result != null) {
				JSONObject	TemplateObject = result.optJSONObject("content");
				FileUtil.saveFile(TemplateObject.toString(), MyConstants.TEMPLATE,
						mTemplateId, MyConstants.JSON);
				getTemplateData(TemplateObject);
			} else {
				showLoading(VIEW_LOADFAILURE);
			}
		} catch (Exception e) {
...
//		AutomaticUtil.reviseTemplate(mTemplateId,data);
//		ToastUtils.show("修复Ok");
			...
			public void run() {
				try {
					String json = FileUtil.readFile(MyConstants.TEMPLATE,
							mTemplateId, MyConstants.JSON);
					JSONObject TemplateObject = new JSONObject(json);
					getTemplateData(TemplateObject);
				} catch (JSONException e) {
					e.printStackTrace();
					showLoading(VIEW_LOADFAILURE);
