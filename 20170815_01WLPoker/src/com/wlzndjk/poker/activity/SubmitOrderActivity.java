...
import com.wlzndjk.poker.adapter.ChoiceShippingListAdapter;
import com.wlzndjk.poker.adapter.SubmitOrderHolder;
import com.wlzndjk.poker.base.BaseActivity;
import com.wlzndjk.poker.bean.ShoppingInfoBean;
import com.wlzndjk.poker.bean.WorksInfoBean;
...
import com.wlzndjk.poker.database.WorksSQLHelper;
import com.wlzndjk.poker.dialog.CouponDialog;
import com.wlzndjk.poker.dialog.CouponDialog.CouponCallBack;
import com.wlzndjk.poker.dialog.CustomDialog;
...
import com.wlzndjk.poker.widget.AddAndSubView;
import com.wlzndjk.poker.widget.AddAndSubView.OnNumChangeListener;
...
public class SubmitOrderActivity extends BaseActivity implements
	...
	private String mMyCoupon = null;
	private String mCouponStr = null;
	...
	public void onClick(View view) {
		...
		case R.id.select_iv:
			if (mIsUseCoupon) {
				mIsUseCoupon = false;
				mSelectIv.setImageResource(R.drawable.checkbox_normal);
				mCouponStr = "";
				initGetPrice();
			} else {
				mIsUseCoupon = true;
				mSelectIv.setImageResource(R.drawable.checkbox_pressed);
				if (!StringUtils.isEmpty(mMyCoupon)) {
					mCouponStr = mMyCoupon;
					initCouponInfo(mCouponStr);
				} else {
					initGetPrice();
				}
			}
	...
	private void couponDialogShow() {
		...
		dialog.setCallBack(new CouponCallBack() {
			@Override
			public void isConfirm(String callBackData) {
				if (!StringUtils.isEmpty(callBackData)) {
					mMyCoupon = callBackData;
					if (StringUtils.isEmpty(callBackData)) {
						mCouponTv.setText("");
						initGetPrice();
					} else {
						initCouponInfo(callBackData);
					}
				}
			}
	...
	private void initCouponInfo(String callBackData) {
		// TODO Auto-generated method stub
		NetHelper.getCouponInfo(callBackData, new AsyncHttpResponseHandler() {

			@Override
			public void onSuccess(int statusCode, Header[] headers,
					byte[] responseBody) {
				LogUtils.logd(TAG, "initCouponInfoSuccess");
				resolveCouponInfo(responseBody);
			...
			public void onFailure(int arg0, Header[] arg1, byte[] arg2,
					Throwable arg3) {
				// TODO Auto-generated method stub
				LogUtils.logd(TAG, "initCouponInfoFailure");
	...
	private void resolveCouponInfo(byte[] responseBody) {
		// TODO Auto-generated method stub
		try {
			String json = new String(responseBody, "UTF-8");
			JSONObject JSONObject = new JSONObject(json);
			LogUtils.log(TAG, "CouponInfo:" + json);
			String code = JSONObject.optString("code");
			String msg = JSONObject.optString("msg");
			JSONObject result = JSONObject.optJSONObject("result");
			if (code.equals("0")) {
				if (result != null) {
					String coupon_name = result.optString("coupon_name");
					mIsAvailable = result.optBoolean("is_available");
					LogUtils.logd(TAG, "mIsAvailable:" + mIsAvailable);
					String coupon_num = result.optString("coupon_num");
					if (mIsAvailable) {
						mMyCoupon = coupon_num;
						mCouponTv.setText(coupon_name);
					} else {
						ToastUtils.show(R.string.coupon_no_available);
						mCouponStr = "";
						mMyCoupon = "";
						mCouponTv.setText("");
					}
	...
	private void initView() {
		...
		mCouponTv = (TextView) mFooter.findViewById(R.id.coupon_tv);
		mSelectIv = (ImageView) mFooter.findViewById(R.id.select_iv);
	...
	private void initGetPrice() {
		...
		// mCouponStr = mMyCoupon;
		// }
		if (mIsUseCoupon & mIsAvailable) {
			mCouponStr = mMyCoupon;
		} else {
			mCouponStr = "";
		}
		LogUtils.logd(TAG, "mCouponStr:" + mCouponStr);
		NetHelper.getCheckCost(mAccountId, mAccountToken, mShippingId,
				mAddressIdStr, mCouponStr, mObjects,
				new AsyncHttpResponseHandler() {
	...
	private void submitOrder() {
		...
		NetHelper.createOrders(mAccountId, mAccountToken, mAddressIdStr,
				mShippingId, mObjects, recommended_code, memo, mCouponStr,
				new AsyncHttpResponseHandler() {
