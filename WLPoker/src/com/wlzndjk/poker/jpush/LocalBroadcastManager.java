package com.wlzndjk.poker.jpush;
...
public final class LocalBroadcastManager {
			...
			if (filters != null) {
				for (int i = 0; i < filters.size(); ++i) {
					...
					for (int j = 0; j < filter.countActions(); ++j) {
						...
						ArrayList receivers = (ArrayList) this.mActions
								.get(action);
						if (receivers != null) {
							for (int k = 0; k < receivers.size(); ++k) {
								if (((LocalBroadcastManager.ReceiverRecord) receivers
	...
	public boolean sendBroadcast(Intent intent) {
		...
		synchronized (this.mReceivers) {
			...
			if (debug) {
				Logger.v("LocalBroadcastManager", "Resolving type " + type
			...
			if (entries != null) {
				...
				for (i = 0; i < entries.size(); ++i) {
					LocalBroadcastManager.ReceiverRecord receiver = (LocalBroadcastManager.ReceiverRecord) entries
							.get(i);
					if (debug) {
						Logger.v("LocalBroadcastManager",
				...
				if (receivers != null) {
					for (i = 0; i < receivers.size(); ++i) {
						((LocalBroadcastManager.ReceiverRecord) receivers
		...
		while (true) {
			...
			for (int var6 = 0; var6 < brs.length; ++var6) {
				...
				for (int j = 0; j < var7.receivers.size(); ++j) {
					((LocalBroadcastManager.ReceiverRecord) var7.receivers
